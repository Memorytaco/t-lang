...
sOut, sIn
  :: Gr (GNode (GNodeLabel lit label name)) (GEdge name) -> Node -> [(Node, Int)]
...
sReach
  :: Gr (GNode (GNodeLabel lit label name)) (GEdge name) -> Node -> Node -> Bool
sReach g n1 n2 = elem n2 . dfs [n1] $ elfilter isStructureEdge g

-- | out structure edge of a node
sEdgeOut, sEdgeIn, bEdgeOut, bEdgeIn :: Gr (GNode (GNodeLabel lit label name)) (GEdge name) -> Node -> [LEdge (GEdge name)]
